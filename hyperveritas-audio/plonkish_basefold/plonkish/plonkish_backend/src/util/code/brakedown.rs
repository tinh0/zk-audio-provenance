...
impl<F: Field> SparseMatrix<F> {
    fn new(dimension: SparseMatrixDimension, mut rng: impl RngCore) -> Self {
        // Use u64 for column sampling to ensure consistent RNG consumption
        // across 32-bit (WASM) and 64-bit (native) targets.
        let dist = Uniform::new(0u64, dimension.m as u64);
