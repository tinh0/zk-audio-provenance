...
/// Convert audio samples to field elements with bit-depth-aware offset.
///
/// For signed audio formats (16-bit and 24-bit), samples are offset to unsigned
/// to work with the existing range check infrastructure.
///
/// # Arguments
/// * `samples` - Slice of audio samples as i32
/// * `bit_depth` - Bit depth of the audio (8, 16, or 24)
///
/// # Returns
/// Vector of field elements representing the audio samples
pub fn audio_to_field_vec<F: PrimeField>(samples: &[i32], bit_depth: u8) -> Vec<F> {
    let offset: i64 = match bit_depth {
        8 => 0,           // 8-bit WAV is unsigned [0, 255]
        16 => 32768,      // Offset signed [-32768, 32767] to unsigned [0, 65535]
        24 => 8388608,    // Offset signed [-8388608, 8388607] to unsigned [0, 16777215]
...
/// Get the maximum value for a given audio bit depth.
///
/// # Arguments
/// * `bit_depth` - Bit depth of the audio (8, 16, or 24)
///
/// # Returns
/// Maximum unsigned value for the bit depth
pub fn get_audio_max_val(bit_depth: u8) -> u32 {
    match bit_depth {
        8 => 255,
        16 => 65535,
        24 => 16777215,
