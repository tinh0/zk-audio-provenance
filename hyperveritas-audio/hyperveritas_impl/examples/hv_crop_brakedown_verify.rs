...
use plonkish_backend::{
    ...
    util::{
        ...
        code::{Brakedown, BrakedownSpec3, BrakedownSpec6, LinearCodes},
...
fn setup(input_size: usize) -> (<Pcs as PolynomialCommitmentScheme<F>>::ProverParam, <Pcs as PolynomialCommitmentScheme<F>>::VerifierParam, Vec<Vec<F>>){
    ...
    println!("PCS PARAMS: num_vars={}, num_rows={}, row_len={}, codeword_len={}, num_col_open={}, num_prox={}",
        vp.num_vars, vp.num_rows,
        vp.brakedown.row_len(), vp.brakedown.codeword_len(),
        vp.brakedown.num_column_opening(), vp.brakedown.num_proximity_testing());
...
fn verify_from_bytes(
    ...
    proof_bytes: &[u8],
    crop_image_path: Option<&str>) -> bool {

    println!("\nstarting verifier");
    let width = endX - startX;
    let height = endY - startY;

    // Build eval vec structures (identity-indexed, values will be overwritten from transcript)
    ...
    let fileName = match crop_image_path {
        Some(p) => p.to_string(),
        None => format!("images/Crop{}.json", numCols),
    };
...
fn run_verifier_only(input_size: usize, input_dir: &str, crop_image_path: Option<&str>) {
    let numCols = input_size;
    let nvCrop = input_size - 1;
    let numRows = 7;

    // Re-derive verifier params and camera hash (setup is deterministic)
    let (_pp, vp, digestRGB) = setup(input_size);
    ...
    let origWidth = public_json["origWidth"].as_u64().unwrap() as usize;
    let origHeight = public_json["origHeight"].as_u64().unwrap() as usize;
    let startX = public_json["startX"].as_u64().unwrap() as usize;
    let startY = public_json["startY"].as_u64().unwrap() as usize;
    let endX = public_json["endX"].as_u64().unwrap() as usize;
    let endY = public_json["endY"].as_u64().unwrap() as usize;

    println!("Loaded proof: {} bytes", proof_bytes.len());
    println!("Public inputs: origWidth={}, origHeight={}, crop=({},{})..({},{})",
        origWidth, origHeight, startX, startY, endX, endY);
    if let Some(p) = crop_image_path {
        println!("Using custom crop image: {}", p);
    }
    ...
    let success = verify_from_bytes(
        vp, numRows, numCols, nvCrop,
        origWidth, origHeight, startX, startY, endX, endY,
        digestRGB, &proof_bytes, crop_image_path,
    );
...
fn main(){
    ...
    if args.len() < 3 {
        eprintln!("Usage: hv_crop_brakedown_verify <size> <input_dir> [crop_image_path]");
        std::process::exit(1);
    }

    let input_size = args[1].parse::<usize>().unwrap();
    let input_dir = &args[2];
    let crop_image_path = if args.len() >= 4 { Some(args[3].as_str()) } else { None };

    println!("-----------------------------------------------------------------------");
    println!("Verifier Only - Crop, HyperVerITAS Brakedown 127. Size: 2^{:?}\n", input_size);
    run_verifier_only(input_size, input_dir, crop_image_path);
    println!("-----------------------------------------------------------------------");
}