...
fn setup(input_size: usize) -> (<Pcs as PolynomialCommitmentScheme<F>>::ProverParam, <Pcs as PolynomialCommitmentScheme<F>>::VerifierParam, Vec<Vec<F>>) {
    ...
    let leftEvals = audio_to_brakedown_field(&stereoAudio.left, bitDepth);
    let rightEvals = audio_to_brakedown_field(stereoAudio.right.as_ref().unwrap(), bitDepth);
...
fn prove(pp: <Pcs as PolynomialCommitmentScheme<F>>::ProverParam, input_size: usize, numRows: usize, numCols: usize)
 -> (Vec<Vec<Evaluation<F>>>, Vec<Vec<Evaluation<F>>>, Vec<Vec<Evaluation<F>>>,
     Vec<Vec<Evaluation<F>>>, Vec<Vec<Evaluation<F>>>,
     ...
     (impl (TranscriptWrite<<Pcs as PolynomialCommitmentScheme<F>>::CommitmentChunk, F>) + InMemoryTranscript))
{
    ...
    let monoEvals = audio_to_brakedown_field(&monoAudio.left, bitDepth);
    ...
    // --- Left channel evaluations (3 points: hash, range_small, transform) ---
    let leftInd = 0;
    let mut leftcom_0: Vec<&_> = (0..3).map(|_| &coms[leftInd]).collect();
    let mut leftpoly_0: Vec<&_> = (0..3).map(|_| &polynomials[leftInd]).collect();
    let mut smallPt = points[3].clone();
    smallPt[numCols] = F::ZERO;
    let leftpoints_0 = vec![points[0].clone(), smallPt.clone(), points[9].clone()];
    let mut leftevals_0 = Vec::new();
    leftevals_0.push(Evaluation::new(0, 0, leftpoly_0[0].evaluate(&leftpoints_0[0])));
    leftevals_0.push(Evaluation::new(1, 1, leftpoly_0[1].evaluate(&leftpoints_0[1])));
    leftevals_0.push(Evaluation::new(2, 2, leftpoly_0[2].evaluate(&leftpoints_0[2])));

    let mut leftevals_vec = vec![leftevals_0];

    // --- Right channel evaluations (2 points: hash, transform) ---
    let rightInd = 1;
    let mut rightcom_0: Vec<&_> = (0..2).map(|_| &coms[rightInd]).collect();
    let mut rightpoly_0: Vec<&_> = (0..2).map(|_| &polynomials[rightInd]).collect();
    let rightpoints_0 = vec![points[0].clone(), points[9].clone()];
    let mut rightevals_0 = Vec::new();
    rightevals_0.push(Evaluation::new(0, 0, rightpoly_0[0].evaluate(&rightpoints_0[0])));
    rightevals_0.push(Evaluation::new(1, 1, rightpoly_0[1].evaluate(&rightpoints_0[1])));

    let mut rightevals_vec = vec![rightevals_0];
    ...
    // Left channel
    transcript.write_field_elements(leftevals_vec[0].iter().map(Evaluation::value)).unwrap();
    batch_open_one::<F, Pcs>(&pp, length, leftpoly_0, leftcom_0, &leftpoints_0, &leftevals_vec[0], &mut transcript).unwrap();

    // Right channel
    transcript.write_field_elements(rightevals_vec[0].iter().map(Evaluation::value)).unwrap();
    batch_open_one::<F, Pcs>(&pp, length, rightpoly_0, rightcom_0, &rightpoints_0, &rightevals_vec[0], &mut transcript).unwrap();
    ...
    return (hevals_vec, fracevals_vec, prodevals_vec, leftevals_vec, rightevals_vec, errevals_vec, hErrevals_vec, fracErrevals_vec, prodErrevals_vec, transcript);
}
...
fn verify(
    ...
    leftevals_vec: Vec<Vec<Evaluation<F>>>,
    rightevals_vec: Vec<Vec<Evaluation<F>>>,
    ...
    batch_verify_one::<F, Pcs>(&vp, numCols+1, commits[2].clone(), &hpoints_0, &hevals2, &mut ver_transcript).unwrap();
    ...
    batch_verify_one::<F, Pcs>(&vp, numCols+1, commits[3].clone(), &fracpoints_0, &fracevals2, &mut ver_transcript).unwrap();
    ...
    batch_verify_one::<F, Pcs>(&vp, numCols+1, commits[4].clone(), &prodpoints_0, &prodevals2, &mut ver_transcript).unwrap();

    // --- Left channel ---
    let mut smallPt = points[3].clone();
    smallPt[numCols] = F::ZERO;
    let leftpoints_0 = vec![points[0].clone(), smallPt.clone(), points[9].clone()];
    let left_evals: Vec<F> = ver_transcript.read_field_elements(leftevals_vec[0].len()).unwrap();
    let mut leftevals2 = Vec::new();
    for j in 0..leftevals_vec[0].len() {
        let mut newEval = leftevals_vec[0][j].clone();
        newEval.value = left_evals[j];
        leftevals2.push(newEval);
    }
    batch_verify_one::<F, Pcs>(&vp, numCols+1, commits[0].clone(), &leftpoints_0, &leftevals2, &mut ver_transcript).unwrap();

    // --- Right channel ---
    let rightpoints_0 = vec![points[0].clone(), points[9].clone()];
    let right_evals: Vec<F> = ver_transcript.read_field_elements(rightevals_vec[0].len()).unwrap();
    let mut rightevals2 = Vec::new();
    for j in 0..rightevals_vec[0].len() {
        let mut newEval = rightevals_vec[0][j].clone();
        newEval.value = right_evals[j];
        rightevals2.push(newEval);
    }
    batch_verify_one::<F, Pcs>(&vp, numCols+1, commits[1].clone(), &rightpoints_0, &rightevals2, &mut ver_transcript).unwrap();
    ...
    batch_verify_one::<F, Pcs>(&vp, numCols+1, commits[5].clone(), &errpoints_0, &errevals2, &mut ver_transcript).unwrap();
    ...
    batch_verify_one::<F, Pcs>(&vp, numCols+1, commits[6].clone(), &hErrpoints_0, &hErrevals2, &mut ver_transcript).unwrap();
    ...
    batch_verify_one::<F, Pcs>(&vp, numCols+1, commits[7].clone(), &fracErrpoints_0, &fracErrevals2, &mut ver_transcript).unwrap();
    ...
    batch_verify_one::<F, Pcs>(&vp, numCols+1, commits[8].clone(), &prodErrpoints_0, &prodErrevals2, &mut ver_transcript).unwrap();
    ...
    // --- Mono transformation check (SOUND) ---
    // Verify: left(alpha) + right(alpha) == 2 * mono(alpha) + error(alpha)
    let leftAtPt = left_evals[2]; // left at transform point (PCS-opened)
    let rightAtPt = right_evals[1]; // right at transform point (PCS-opened)
...
fn run_whole_system_mono_brake(input_size: usize) {
    ...
    let (hevals_vec, fracevals_vec, prodevals_vec, leftevals_vec, rightevals_vec, errevals_vec, hErrevals_vec, fracErrevals_vec, prodErrevals_vec, transcript) = prove(pp, input_size, numRows, numCols);
    let elapsed_prover = prover_start.elapsed();
    println!("PROVER TIME: {:?} seconds", elapsed_prover.as_millis() as f64 / 1000.0);

    // verify
    let verifier_start = Instant::now();
    verify(vp, numRows, numCols, hevals_vec, fracevals_vec, prodevals_vec, leftevals_vec, rightevals_vec, errevals_vec, hErrevals_vec, fracErrevals_vec, prodErrevals_vec, digest, transcript, input_size);
...
fn main() {
    ...
    for i in first_size..last_size+1 {
        println!("-----------------------------------------------------------------------");
        println!("Full System Mono, HyperVerITAS Brakedown 127. Size: 2^{:?}\n", i);
        let _res = run_whole_system_mono_brake(i);
        println!("-----------------------------------------------------------------------");
    }
}
