...
fn run_prover_only(input_size: usize, output_dir: &str) {
    ...
    let (pp, _vp, digestRGB) = setup(input_size);

    // now we begin proving
    let prover_start = Instant::now();

    let (origWidth, origHeight, startX, startY, endX, endY, _hevals_vec, _fracevals_vec, _prodevals_vec, _imgevals_vec, transcript) = prove(pp, input_size, numRows, numCols, nvCrop);
    ...
    // Write camera hash (digestRGB) — this is the public attestation the "camera" would sign
    fs::write(
        out_path.join("camera_hash.json"),
        serde_json::to_string(&digestRGB).unwrap(),
...
fn main(){
    ...
    if args.len() < 3 {
        eprintln!("Usage: hv_crop_brakedown_prove <size> <output_dir>");
        std::process::exit(1);
    }
    ...
    println!("-----------------------------------------------------------------------");
    println!("Prover Only - Crop, HyperVerITAS Brakedown 127. Size: 2^{:?}\n", input_size);
