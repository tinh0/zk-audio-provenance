...
static name : &str = "oneChannel_Hash";
...
fn get_instance(length_0 : usize, length_1 : usize, length_2 : usize, degree_bits : usize, zeta : QuadraticExtension<F>, pixel_count: usize) -> FriInstanceInfo<F, D> {
    ...
    let pixels = g.exp_u64((pixel_count) as u64);
    ...
    let pixels_plus_pixel_range = g.exp_u64((pixel_count + PIXEL_RANGE as usize) as u64);
...
pub fn verify(fri_openings: FriOpenings<F, D>, 
          merkle_caps: [MerkleCap<GoldilocksField, PoseidonHash>; 3], 
          opening_proof: FriProof<F, <PoseidonGoldilocksConfig as GenericConfig<D>>::Hasher, D>, 
          mut last: u128,
          rate_bits: usize,
          cap_height: usize,
          degree_bits: usize,
    ...
    let instance = get_instance(3, 7, 3, degree_bits, zeta, pixels);
...
fn run_one_hash(size: &usize) {
    println!("One Channel Hash , VerITAS FRI. Size: 2^{:?}\n", size);
    let now = Instant::now();
    ...
    prove(start, start, 2, 4, omegaR, &fft_root_tableR, "R", &size, pixels, degree);
