// trim proves that a public trimmed segment came from secret original audio,
// with MiMC hash preimage binding.
// Default: trims first half of audio (matching hyperveritas_impl behavior).
...
func main() {
	...
	if len(positional) < 1 {
		fmt.Fprintln(os.Stderr, "Usage: trim [--plonk] <size> [n_samples]")
		os.Exit(1)
	}
	...
	fmt.Printf("Backend: %s, Samples: %d, Trim: %d (authentic)\n", backend, n, trimLen)

	// Convert samples to float32 bit patterns
	inputBits := make([]uint32, n)
	inputVars := make([]frontend.Variable, n)
	outputVars := make([]frontend.Variable, trimLen)
	for i, s := range samples {
		...
		if i < trimLen {
			outputVars[i] = inputBits[i]
		}
	}

	// Compute MiMC digest of all inputs
	...
	// Compile
	circuit := &zkAudio.AudioTrimCircuit{
		Inputs:  make([]frontend.Variable, n),
		Outputs: make([]frontend.Variable, trimLen),
	}

	fmt.Printf("Compiling AudioTrimCircuit (N=%d, M=%d, %s)... ", n, trimLen, backend)
	t0 = time.Now()
	...
	if usePlonk {
		...
		fmt.Printf("%v (%d constraints, %.1f/sample)\n", compileTime, nbConstraints, float64(nbConstraints)/float64(trimLen))
		...
		assignment := &zkAudio.AudioTrimCircuit{
			Inputs: inputVars, InputDigest: digest, Outputs: outputVars,
		}
		...
		fmt.Printf("%v (%.1f μs/sample)\n", proveTime, float64(proveTime.Microseconds())/float64(trimLen))
	...
	writeCSV("trim", backend, size, n, trimLen, nbConstraints, compileTime, setupTime, proveTime, verifyTime)

	fmt.Printf("Summary (%s)\n", backend)
	fmt.Printf("  Input samples: %d\n", n)
	fmt.Printf("  Trim samples:  %d\n", trimLen)
	fmt.Printf("  Constraints:   %d (%.1f/sample)\n", nbConstraints, float64(nbConstraints)/float64(trimLen))
...
func writeCSV(name, backend string, size, inputN, trimN, constraints int, compile, setup, prove, verify time.Duration) {
	os.MkdirAll("output", 0755)
	csvPath := fmt.Sprintf("output/%s_%s_%d_n%d.csv", name, backend, size, trimN)
	f, _ := os.Create(csvPath)
	defer f.Close()
	w := csv.NewWriter(f)
	w.Write([]string{"Backend", "InputSize", "TrimSize", "NbConstraints", "ConstraintsPerSample",
		"CompileTime_ms", "SetupTime_ms", "ProveTime_ms", "VerifyTime_ms", "ProveTimePerSample_us"})
	w.Write([]string{backend, strconv.Itoa(inputN), strconv.Itoa(trimN), strconv.Itoa(constraints),
		fmt.Sprintf("%.2f", float64(constraints)/float64(trimN)),
		...
		fmt.Sprintf("%.2f", float64(prove.Microseconds())/float64(trimN))})
	w.Flush()
	fmt.Printf("  CSV:           %s\n", csvPath)
}
