// authentic_fade_out proves fade-out with MiMC hash preimage binding.
...
func main() {
	...
	if len(positional) < 1 {
		fmt.Fprintln(os.Stderr, "Usage: authentic_fade_out [--plonk] <size> [n_samples]")
		os.Exit(1)
	}
	...
	fmt.Printf("Backend: %s, Samples: %d (authentic fade out)\n", backend, n)

	// Convert samples and compute outputs: output[i] = input[i] * (1 - i/n)
	...
	for i, s := range samples {
		...
		factor := 1.0 - float32(i)/float32(n)
		outF := inF * factor
	...
	circuit := &zkAudio.AudioFadeOutCircuit{
		Inputs:  make([]frontend.Variable, n),
		Outputs: make([]frontend.Variable, n),
	}

	fmt.Printf("Compiling AudioFadeOutCircuit (N=%d, %s)... ", n, backend)
	t0 = time.Now()
	...
	if usePlonk {
		...
		assignment := &zkAudio.AudioFadeOutCircuit{
	...
	writeCSV("authentic_fade_out", backend, size, n, nbConstraints, compileTime, setupTime, proveTime, verifyTime)
