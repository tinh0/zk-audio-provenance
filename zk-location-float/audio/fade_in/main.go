// authentic_fade_in proves fade-in with MiMC hash preimage binding.
...
func main() {
	...
	if len(positional) < 1 {
		fmt.Fprintln(os.Stderr, "Usage: authentic_fade_in [--plonk] <size> [n_samples]")
		os.Exit(1)
	}
	...
	fmt.Printf("Backend: %s, Samples: %d (authentic fade in)\n", backend, n)

	// Convert samples and compute outputs: output[i] = input[i] * (i / n)
	...
	circuit := &zkAudio.AudioFadeInCircuit{
		Inputs:  make([]frontend.Variable, n),
		Outputs: make([]frontend.Variable, n),
	}

	fmt.Printf("Compiling AudioFadeInCircuit (N=%d, %s)... ", n, backend)
	t0 = time.Now()
	...
	if usePlonk {
		...
		assignment := &zkAudio.AudioFadeInCircuit{
	...
	writeCSV("authentic_fade_in", backend, size, n, nbConstraints, compileTime, setupTime, proveTime, verifyTime)
