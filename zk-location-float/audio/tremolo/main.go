// authentic_tremolo proves tremolo with MiMC hash preimage binding.
// Usage: go run main.go [--plonk] <size> [n_samples] [rate_hz] [depth]
package main
...
func main() {
	...
	if len(positional) < 1 {
		fmt.Fprintln(os.Stderr, "Usage: authentic_tremolo [--plonk] <size> [n_samples] [rate_hz] [depth]")
		os.Exit(1)
	}
	...
	depth := float32(0.5)
	if len(positional) >= 4 {
		d, _ := strconv.ParseFloat(positional[3], 32)
		depth = float32(d)
	}
	...
	fmt.Printf("Backend: %s, Rate: %.2f Hz, Depth: %.2f, Samples: %d (authentic)\n", backend, rate, depth, n)
	...
	// Compile
	circuit := &zkAudio.AudioTremoloCircuit{
		Inputs:     make([]frontend.Variable, n),
		Outputs:    make([]frontend.Variable, n),
		SampleRate: sr,
	}

	fmt.Printf("Compiling AudioTremoloCircuit (N=%d, %s)... ", n, backend)
	t0 = time.Now()
	...
	if usePlonk {
		...
		assignment := &zkAudio.AudioTremoloCircuit{
			Inputs: inputVars, InputDigest: digest,
			Rate: f32Bits(rate), Depth: f32Bits(depth),
			Outputs: outputVars, SampleRate: sr,
		}
	...
	writeCSV("authentic_tremolo", backend, size, n, nbConstraints, compileTime, setupTime, proveTime, verifyTime)
...
func loadAudio(size int) Audio {
	for _, c := range []string{
		"hyperveritas_impl/audio",
		"../hyperveritas_impl/audio",
		"../../hyperveritas_impl/audio",
		"../../../hyperveritas_impl/audio",
		"../../../hyperveritas-audio/hyperveritas_impl/audio",
	} {
